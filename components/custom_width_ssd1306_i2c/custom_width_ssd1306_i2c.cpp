...
void CustomWidthSSD1306::dump_config() {
  LOG_DISPLAY("", "I2C SSD1306 (custom width)", this);
  ESP_LOGCONFIG(TAG,
                "  Model: %s\n"
                "  Custom Width: %d\n"
                "  External VCC: %s\n"
                ...
                "  Offset Y: %d\n"
                "  Inverted Color: %s",
                LOG_STR_ARG(this->model_str_()), this->custom_width_, YESNO(this->external_vcc_),
                YESNO(this->flip_x_), YESNO(this->flip_y_), this->offset_x_, this->offset_y_, YESNO(this->invert_));
  LOG_I2C_DEVICE(this);
...
void CustomWidthSSD1306::command(uint8_t value) { this->write_byte(0x00, value); }

void HOT CustomWidthSSD1306::write_display_data() {
